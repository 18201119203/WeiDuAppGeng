...
public class ShopYiangActivity extends BasemvpActivity<RetrofitContract.IretrofitModel,RetrofitContract.retrofitPresenter> implements MyScrollView.ScrollviewListener,RetrofitContract.IretrofitView {
    ...
    public void addCart(){
        ...
        presenter.getCardData(Api.SHOPCART+"?userId="+userId+"&sessionId="+sessionId,hashMap);
    ...
    public  void SuccessData(Object data) {
        if(data instanceof ShopCartBean){
            ...
            List<ShopCartBean.ResultBean> list = shopCartBean.result;
            String rderInfo = "";

            for (int i = 0; i < list.size(); i++) {
                if (list.size()==1) {
                    rderInfo = "["+list.get(i).toStringData();
            ...
            Log.e("ssssssssssssssss", rderInfo);
        //加
        shopData.result.setCount(1);
        rderInfo += shopData.result.toString();
        HashMap<String, String> bodyparams = new HashMap<>();
        bodyparams.put("data",rderInfo);
